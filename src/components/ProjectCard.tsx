        ...
        >
          ...
          >
            ...
            <div className="absolute top-0 left-0 w-full h-full z-20 flex items-end justify-center">
              <div
                className={`bg-linear-to-b from-transparent to-45% to-black
                w-full flex flex-col gap-2 md:gap-3 group-hover:opacity-100 translate-y-full group-hover:translate-y-0 opacity-0 transition-all duration-500 group-hover:ease-out ease-in p-5 md:p-8`}
              >
                ...
                <div
                  className={`hero-heading text-white text-lg md:text-4xl italic font-thin tracking-[-0.03em] leading-[100%]`}
                >
                  {data.name}
                </div>
                <div className={`text-white/70 text-[8px] md:text-sm font-light tracking-tight space-y-1.5 md:space-y-2`}>
                  {data.points.map((point, i) => (
                    <div
                      key={i}
                      className={`
                      ${i === data.points.length - 1
                          ? "text-(--color-design) w-fit tracking-[0.05em] uppercase text-[7px] md:text-xs"
                          : ""
                        }
                      `}
                    >
